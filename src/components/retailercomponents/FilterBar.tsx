        ...
        <div className="flex items-center gap-2 ml-auto">
          ...
          <div className="flex items-center -space-x-2">
            {connections.map((conn) => {
              const isActive = selectedWholesalerId === conn.wholesalerId;
              const productCount = products.filter((p) => p.wholesalerId === conn.wholesalerId).length;

              return (
                <div key={conn.wholesalerId} className="relative group">
                  <button
                    onClick={() =>
                      setSelectedWholesalerId((prev) => (prev === conn.wholesalerId ? null : conn.wholesalerId))
                    }
                  ...
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:flex flex-col items-center z-50 pointer-events-none">
                    <div className="bg-gray-900 text-white text-xs rounded-lg px-3 py-2 whitespace-nowrap shadow-xl">
                      <p className="font-semibold">{conn.wholesaler?.name}</p>
          ...
          {selectedWholesalerId && (
            <span className="text-xs text-[#006989] font-medium bg-blue-50 px-2 py-1 rounded-full border border-[#006989]/30">
              {connections.find((c) => c.wholesalerId === selectedWholesalerId)?.wholesaler?.name}
            </span>
          )}
        </div>
      )}
