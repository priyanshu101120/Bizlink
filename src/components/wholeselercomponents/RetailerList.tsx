...
const RetailerList = ({ connections, allRetailers, onConnect, onDisconnect }: Props) => {
    ...
    <div className="space-y-3">
      ...
      {connections.length > 0 && (
        <div className="bg-white rounded-2xl border border-[#006989] overflow-hidden">
          <div className="px-4 py-3 border-b border-gray-200">
            <p className="text-sm font-medium">Connected Retailers ({connections.length})</p>
          </div>
          {connections.map((conn) => (
            <div key={conn.id} className="flex items-center justify-between px-4 py-3 border-b last:border-0">
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        <div className="px-5 py-3 border-b border-gray-100">
          <p className="text-sm font-medium text-gray-700">Connect to more retailers</p>
        </div>
        {unconnectedRetailers.length === 0 ? (
          <p className="text-center py-8 text-gray-400 text-sm">No retailers available</p>
        ) : (
          unconnectedRetailers.map((retailer) => (
            <div key={retailer.id} className="flex items-center justify-between px-4 py-4 border-b last:border-0">
              <div className="flex items-center gap-3">
